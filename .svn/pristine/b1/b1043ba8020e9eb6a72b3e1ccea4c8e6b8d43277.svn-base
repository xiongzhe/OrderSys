...
+ (void)initialize
{
	if (self == [ASIHTTPRequest class]) {
        ...
        NSString *localString = NSLocalizedString(@"The request timed out", @"The request timed out");
		ASIRequestTimedOutError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:ASIRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:localString,NSLocalizedDescriptionKey,nil]];  
...

{
	...
	if (![self error]) { // We may already have handled this error
		...
		NSString *reason = NSLocalizedString(@"A connection failure occurred",@"A connection failure occurred");
