 ...
 * 收益分析
 **/
@interface ProfitViewController ()<TimeViewDelegate>
    ...
    dispatch_async(dispatch_queue_create("getclass", DISPATCH_QUEUE_CONCURRENT), ^(){
        ...
        if (dics!=nil) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [self hideHud];
                if (TotalIncome != 0) {
                    _incomeText.text = [NSString stringWithFormat:@"+%d", TotalIncome];
                } else {
                    _incomeText.text = [NSString stringWithFormat:@"%d", TotalIncome];
                }
                _spendText.text = [NSString stringWithFormat:@"%d", TotalExpend];
                _grossText.text = [NSString stringWithFormat:@"%d", TotalProfit];
            });
        } else {
            dispatch_async(dispatch_get_main_queue(), ^{
                [self hideHud];
