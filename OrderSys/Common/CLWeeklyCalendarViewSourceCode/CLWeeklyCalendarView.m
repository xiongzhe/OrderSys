...
static NSString* const CLCalendarSelectedDatePrintFormatDefault = @"yyyy年MM月dd日";
...
-(UIView *)dailyInfoSubViewContainer
{
    if(!_dailyInfoSubViewContainer){
        _dailyInfoSubViewContainer = [[UIView alloc] initWithFrame:CGRectMake(25, DATE_TITLE_MARGIN_TOP+DAY_TITLE_VIEW_HEIGHT + DATE_VIEW_HEIGHT + DATE_VIEW_MARGIN * 2, (self.backgroundImageView.frame.size.width-50), DATE_LABEL_INFO_HEIGHT)];
...
{
    if(!_goLeftButton){
        _goLeftButton = [[ UIButton alloc] initWithFrame:CGRectMake(0, DATE_TITLE_MARGIN_TOP+30, 20, DAY_TITLE_VIEW_HEIGHT +DATE_LABEL_INFO_HEIGHT)];
        _goLeftButton.backgroundColor = [UIColor clearColor];
        [_goLeftButton setImage:[UIImage imageNamed:@"goLeft"] forState:UIControlStateNormal];
        [_goLeftButton addTarget:self action:@selector(swipeRight:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _goLeftButton;
}
-(UIButton *)goRightButton
{
    if(!_goRightButton){
        _goRightButton = [[ UIButton alloc] initWithFrame:CGRectMake(self.bounds.size.width-25, _goLeftButton.frame.origin.y, 20, _goLeftButton.frame.size.height)];
        [_goRightButton setImage:[UIImage imageNamed:@"goLeft"]forState:UIControlStateNormal];
        [_goRightButton addTarget:self action:@selector(swipeLeft:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _goRightButton;
}
...
-(UIView *)dayTitleSubViewContainer
{
    ...
    if(!_dayTitleSubViewContainer){
        _dayTitleSubViewContainer = [[UIImageView alloc] initWithFrame:CGRectMake(25 , DATE_TITLE_MARGIN_TOP, (self.backgroundImageView.frame.size.width-50), DAY_TITLE_VIEW_HEIGHT)];
...
-(UIView *)dailySubViewContainer
{
    ...
    if(!_dailySubViewContainer){
        _dailySubViewContainer = [[UIImageView alloc] initWithFrame:CGRectMake(25, DATE_TITLE_MARGIN_TOP+DAY_TITLE_VIEW_HEIGHT+DATE_VIEW_MARGIN, (self.backgroundImageView.frame.size.width-50), DATE_VIEW_HEIGHT)];
...
//        [_backgroundImageView addSubview:self.goLeftButton];
//        [_backgroundImageView addSubview:self.goRightButton];
...
-(void)initDailyViews
{
    CGFloat dailyWidth = (self.backgroundImageView.frame.size.width-50)/WEEKLY_VIEW_COUNT;
...
-(UILabel *)dayTitleViewForDate: (NSDate *)date inFrame: (CGRect)frame
{
    ...
    dayTitleLabel.textColor = RGBColorWithoutAlpha(240, 111, 90);
...
//    if([date isDateToday]){
//        strDate = [NSString stringWithFormat:@"今日, %@", strDate ];
//    }
//    
//    
...
//        titleLabel.date = dt;
        titleLabel.text = [dt getDayOfWeekShortString];
        [view markSelected:([view.date isSameDateWith:self.selectedDate])];
        self.endDate = dt;
        if (i == 0) {
            if ([self.delegate respondsToSelector:@selector(firstDateForView:)]) {
                [self.delegate firstDateForView:dt];
            }
        }
    }
}
